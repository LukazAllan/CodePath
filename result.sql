    ...
    create table lesson_progress (
        ...
        status varchar(255) check ((status in ('LOCKED', 'ACTIVE', 'DONE'))) default 'LOCKED',
    ...
    -- TRIGGERS
    -- =========================================================
    drop trigger if exists trg_create_streak;
    create trigger if not exists trg_create_streak
    ...
    DROP TRIGGER if exists trg_create_lesson_progress_after_enrollment;
    CREATE TRIGGER trg_create_lesson_progress_after_enrollment
    ...
    BEGIN
        ...
        SELECT
            ...
            lesson.id,
            CASE
                WHEN ROW_NUMBER() OVER (ORDER BY section.ordem, lesson.ordem) = 1
                    THEN 'ACTIVE'
                ELSE 'LOCKED'
            END
        FROM lesson
...
-- =+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
-- =+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
-- 1ST COURSE - Trilha Backend Java + Spring Boot
-- =+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
-- =+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
...
-- =+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
-- =+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
-- 2ND COURSE - Rede de Computadores - Fundamentos e Prática
-- =+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
-- =+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+

INSERT INTO course (id, name)
VALUES (2, 'Rede de Computadores - Fundamentos e Prática');
...
INSERT INTO section (id, course_id, title, subtitle, color, icon, ordem)
VALUES
    (4, 2, 'Fundamentos de Redes', 'Conceitos básicos e histórico das redes de computadores', 'blue', '🌐', 1),
    (5, 2, 'Modelos e Protocolos', 'Modelo OSI, TCP/IP e principais protocolos', 'green', '📡', 2),
    (6, 2, 'Infraestrutura e Segurança', 'Equipamentos, topologias e segurança de rede', 'red', '🔒', 3);
...
INSERT INTO lesson (id, section_id, name, content, ordem, active)
VALUES
    (10, 4, 'Introdução a Redes', 'Histórico, definição e importância das redes de computadores.', 1, true),
    (11, 4, 'Classificação de Redes', 'PAN, LAN, MAN, WAN e outras classificações.', 2, true),
    (12, 4, 'Meios de Transmissão', 'Cabos, fibras ópticas e meios sem fio.', 3, true),

    (13, 5, 'Modelo OSI', 'As 7 camadas do modelo OSI e suas funções.', 1, true),
    (14, 5, 'Modelo TCP/IP', 'Arquitetura TCP/IP e comparação com OSI.', 2, true),
    (15, 5, 'Protocolos Essenciais', 'TCP, UDP, HTTP, DNS, DHCP e ICMP.', 3, true),

    (16, 6, 'Equipamentos de Rede', 'Switch, roteador, modem, hub e access point.', 1, true),
    (17, 6, 'Endereçamento IP', 'IPv4, IPv6, máscaras de sub-rede e CIDR.', 2, true),
    (18, 6, 'Segurança de Rede', 'Firewall, criptografia e princípios de segurança.', 3, true);
...
INSERT INTO question (id, type, lesson_id, label, prompt, code, correct, hint)
VALUES
    (46, 'SHORT_TEXT', 10, 'Rede', 'O que é uma rede de computadores?', NULL, NULL, 'Pense em dispositivos interconectados.'),
    (47, 'MULTIPLE_CHOICE', 10, 'ARPANET', 'Qual foi a precursora da internet?', NULL, 1, 'Criada nos EUA na década de 1960.'),
    (48, 'TRUE_FALSE', 10, 'Internet', 'A internet é um exemplo de WAN.', NULL, 0, 'WAN significa Wide Area Network.'),

    (49, 'MULTIPLE_CHOICE', 11, 'LAN', 'O que significa LAN?', NULL, 0, 'Rede de área local.'),
    (50, 'TRUE_FALSE', 11, 'PAN', 'PAN é uma rede pessoal.', NULL, 0, 'Exemplo: Bluetooth.'),
    (51, 'SHORT_TEXT', 11, 'WAN', 'O que significa WAN?', NULL, NULL, 'Redes geograficamente distribuídas.'),

    (52, 'MULTIPLE_CHOICE', 12, 'Fibra Óptica', 'Qual meio de transmissão usa luz para transmitir dados?', NULL, 2, 'Alta velocidade e baixa perda.'),
    (53, 'TRUE_FALSE', 12, 'Cabo UTP', 'Cabo UTP é um meio guiado.', NULL, 0, 'Possui conectores RJ45.'),
    (54, 'MULTIPLE_CHOICE', 12, 'Wi-Fi', 'Qual tecnologia usa ondas de rádio?', NULL, 1, 'Sem fio muito comum.'),

    (55, 'SHORT_TEXT', 13, 'OSI', 'O que significa OSI?', NULL, NULL, 'Modelo de referência de 7 camadas.'),
    (56, 'MULTIPLE_CHOICE', 13, 'Camada Aplicação', 'Qual camada OSI está mais próxima do usuário?', NULL, 3, 'Interage diretamente com softwares.'),
    (57, 'TRUE_FALSE', 13, 'Camada Física', 'A camada física lida com bits e sinais.', NULL, 0, 'Define cabos e conectores.'),

    (58, 'MULTIPLE_CHOICE', 14, 'TCP', 'Qual camada do TCP/IP é responsável pela confiabilidade?', NULL, 1, 'Gerencia conexões.'),
    (59, 'TRUE_FALSE', 14, 'IP', 'IP é um protocolo orientado a conexão.', NULL, 1, 'Não garante entrega, é não confiável.'),
    (60, 'SHORT_TEXT', 14, 'TCP/IP', 'O que significa TCP/IP?', NULL, NULL, 'Conjunto de protocolos da internet.'),

    (61, 'MULTIPLE_CHOICE', 15, 'DNS', 'Qual protocolo resolve nomes para IPs?', NULL, 2, 'Traduz domínios.'),
    (62, 'TRUE_FALSE', 15, 'HTTP', 'HTTP opera na porta 443 por padrão.', NULL, 1, 'Enquanto HTTPS usa 443, HTTP usa 80.'),
    (63, 'SHORT_TEXT', 15, 'DHCP', 'O que faz o protocolo DHCP?', NULL, NULL, 'Atribui IPs automaticamente.'),

    (64, 'MULTIPLE_CHOICE', 16, 'Switch', 'Qual equipamento conecta dispositivos na mesma rede?', NULL, 1, 'Opera na camada 2.'),
    (65, 'TRUE_FALSE', 16, 'Roteador', 'Roteador interliga redes diferentes.', NULL, 0, 'Opera na camada 3.'),
    (66, 'MULTIPLE_CHOICE', 16, 'Gateway', 'Qual equipamento conecta redes com protocolos diferentes?', NULL, 3, 'Faz tradução.'),

    (67, 'SHORT_TEXT', 17, 'IPv4', 'Quantos bits tem um endereço IPv4?', NULL, NULL, 'Formato decimal pontuado.'),
    (68, 'MULTIPLE_CHOICE', 17, 'Máscara', 'Qual máscara de sub-rede /24 equivale em decimal?', NULL, 0, '255.255.255.0'),
    (69, 'TRUE_FALSE', 17, 'IPv6', 'IPv6 possui 128 bits.', NULL, 0, 'Muito mais endereços que IPv4.'),

    (70, 'MULTIPLE_CHOICE', 18, 'Firewall', 'O que faz um firewall?', NULL, 1, 'Controla tráfego de rede.'),
    (71, 'TRUE_FALSE', 18, 'Criptografia', 'Criptografia garante confidencialidade.', NULL, 0, 'Protege dados contra acesso não autorizado.'),
    (72, 'SHORT_TEXT', 18, 'VPN', 'O que significa VPN?', NULL, NULL, 'Cria túnel seguro na internet.');
...
VALUES
    ...
    (49, 'Large Area Network'),
    (49, 'Long Area Network'),
    (49, 'Link Area Network'),
...
INSERT INTO question_accepted_answers (question_id, accepted_answers)
VALUES
    (46, 'Conjunto de dispositivos interconectados que compartilham recursos'),
    (51, 'Wide Area Network'),
    (55, 'Open Systems Interconnection'),
    (60, 'Transmission Control Protocol/Internet Protocol'),
    (63, 'Atribui endereços IP automaticamente aos dispositivos'),
    (67, '32'),
    (67, '32 bits'),
